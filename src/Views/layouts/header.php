    ...
    <div class="container-custom">
        <div class="flex items-center justify-between h-16 md:h-18">
            ...
            <nav class="hidden lg:flex items-center space-x-8 rtl:space-x-reverse">
                <a href="/" class="nav-link-clean <?= $isHome ? 'nav-active' : '' ?>">الرئيسية</a>
                <a href="<?= $linkPrefix ?>#services" class="nav-link-clean">الخدمات</a>
                <a href="<?= $linkPrefix ?>#about" class="nav-link-clean">عن خدمة</a>
                <a href="<?= $linkPrefix ?>#faq" class="nav-link-clean">الأسئلة الشائعة</a>
            </nav>
                ...
                <a href="<?= htmlspecialchars(WHATSAPP_CHANNEL_URL) ?>" target="_blank" rel="noopener noreferrer" 
                   class="hidden xl:inline-flex items-center px-4 py-2 rounded-full bg-green-50 text-green-700 hover:bg-green-100 border border-green-200 text-sm font-medium transition-all duration-300 hover:scale-105">
                    <svg class="w-4 h-4 ml-2" viewBox="0 0 24 24">
                        <path fill="#15803d" d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.465 3.516"/>
                    </svg>
                    انضم إلينا
                </a>
                
                <!-- Request Service Button (Desktop) -->
                <a href="<?= $linkPrefix ?>#request-service" class="hidden md:inline-flex items-center px-5 py-2.5 rounded-full bg-gradient-to-r from-primary-600 to-primary-700 text-white text-sm font-medium transition-all duration-300 hover:from-primary-700 hover:to-primary-800 hover:scale-105 hover:shadow-lg">
    ...
    <div id="mobile-menu" class="lg:hidden fixed inset-x-0 top-16 bg-white backdrop-blur-xl border-b border-gray-200 shadow-2xl transform -translate-y-full opacity-0 transition-all duration-500 ease-out z-40">
        <div class="container-custom py-8">
            ...
            <nav class="flex flex-col space-y-4">
                <a href="/" class="mobile-nav-clean <?= $isHome ? 'mobile-nav-active' : '' ?>">الرئيسية</a>
                <a href="<?= $linkPrefix ?>#services" class="mobile-nav-clean">الخدمات</a>
                <a href="<?= $linkPrefix ?>#about" class="mobile-nav-clean">عن خدمة</a>
                <a href="<?= $linkPrefix ?>#faq" class="mobile-nav-clean">الأسئلة الشائعة</a>
            </nav>
            ...
            <div class="mt-6 pt-6 border-t border-gray-100 space-y-3">
                ...
                <a href="<?= $linkPrefix ?>#request-service" class="w-full inline-flex items-center justify-center px-6 py-3.5 rounded-2xl bg-gradient-to-r from-primary-600 to-primary-700 text-white font-semibold text-base hover:from-primary-700 hover:to-primary-800 transition-all duration-300 transform hover:scale-105 shadow-lg">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Active nav link highlighting (Only on Home Page)
    <?php if ($isHome): ?>
    ...
    document.querySelectorAll('a[href*="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            // Only prevent default if it's a pure anchor link on the same page
            const href = this.getAttribute('href');
            const targetId = href.split('#')[1];
            const target = document.getElementById(targetId);
            
            // If we are on the home page OR the link is just an anchor on the current page
